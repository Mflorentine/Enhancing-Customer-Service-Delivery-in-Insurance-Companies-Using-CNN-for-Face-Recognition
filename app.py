...
def extract_features(img_path,model,detector):
    ...
    # =========================================================================================================
def extract_features1(img,model,detector):
    img = cv2.imread(img)
    results = detector.detect_faces(img)
    # results = detector.detect_faces(img)
    # results = imgpath(img_path,detector)
...
def func_db1(predicted_actor):
    ...
    for k in m: # looping through rows
        if k[1]==predicted_actor:# for each row if fist column == id(predicted person)
            with col1:
                # df = pd.DataFrame(k,columns=["id","name", "address","age","identity_numb","Due Date","photo"])
                # st.title(df)
                id1 = k[0]
                name1 = k[1]
                address1 = k[2]
        ...
        # else:
        #     st.title("Information: From SORAS")
        #     st.write("==============================")
    ...
    for k in m: # looping through rows
        if k[1]==predicted_actor:# for each row if fist column == id(predicted person)
            ...
            with col3:
                ...
                st.image(img, use_column_width = True)
        # else:
        #     st.title("Information: From PRIMEINSURANCE")
        #     st.write("==============================================")
        #     st.subheader("NOT FOUND")
    ...
    for k in m: # looping through rows
        if k[1]==predicted_actor:# for each row if fist column == id(predicted person)
            ...
            with col3:
                ...
                st.image(img, use_column_width = True)
        # else:
        #     st.title("Information: From RSSB")
        #     st.write("==============================")
        #     st.subheader("NOT FOUND")
